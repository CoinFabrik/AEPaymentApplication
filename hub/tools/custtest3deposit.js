...
(async function () {
    ...
    try {
        ...
        const X = await peer.channel.deposit(10, async (tx) => {
            console.log("siging deposit.....")
            return await peer.nodeuser.signTransaction(tx);
        });
        console.log(new Date());
        console.log("X is:", X, JSON.stringify(X))

        const Y = await peer.channel.withdraw(10, async (tx) => {
            console.log("siging withdraw.....")
            return await peer.nodeuser.signTransaction(tx);
        });
        console.log(new Date());
        console.log("Y is:", Y, JSON.stringify(Y))
